 ...
 * Colombia) Departamento de Ingeniería de Sistemas Licenciado bajo el esquema Academic Free License
 * version 2.1
 *
 * <p>Proyecto: Parqueadero Adaptado del Proyecto CUPI2 - Uniandes
 * ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 */
...
public class Parqueadero {
  ...
  /**
   * Es el código de error para cuando ya hay un carro en el parqueadero con la placa de un nuevo
   ...
   * <b>post: </b> Se creó un parqueadero abierto con la tarifa establecida y el arreglo de puestos
   * está creado.
   */
  public Parqueadero() {
   ...
   * @return Mensaje con la placa. Si no hay un carro en dicha posición retorna un mensaje indicando
   *     que no hay un carro en esa posición.
   */
  ...
  /**
   * Busca un puesto libre en el parqueadero y lo retorna. Si no encuentra retorna el valor
   * NO_HAY_PUESTO.
   *
  ...
  /**
   * Indica el número de puesto en el que se encuentra el carro con una placa dada. Si no lo
  ...
  /**
   * Avanza una hora en el parqueadero. Si la hora actual es igual a la hora de cierre, el
   * parqueadero se cierra.
   */
  public void avanzarHora() {
    if (this.horaActual == HORA_CIERRE) {
      this.abierto = false;
      return;
    }
    this.horaActual++;
   ...
   *
   * @param pPuesto El puesto que se quiere saber si est? ocupado. pPuesto >= 0 && pPuesto <
   *     puestos.length.
  ...
  public String metodo1() {
    ArrayList<Puesto> puestosOcupados = this.obtenerPuestosOcupados();

    String placaMayor = "No hay carros";
    int tiempoMayor = 0;

    for (Puesto puesto : puestosOcupados) {
      Carro carro = puesto.darCarro();
      int tiempoCarro = carro.darTiempoEnParqueadero(this.horaActual);
      if (tiempoCarro >= tiempoMayor) {
        tiempoMayor = tiempoCarro;
  ...
  private ArrayList<Puesto> obtenerPuestosOcupados() {
    ArrayList<Puesto> puestosOcupados = new ArrayList<Puesto>();

    for (int i = 0; i < TAMANO; i++) {
      if (this.estaOcupado(i)) {
        puestosOcupados.add(this.puestos[i]);
      }
    }
  ...
  public String metodo2() {
    ArrayList<Puesto> puestosOcupados = this.obtenerPuestosOcupados();
    if (puestosOcupados.size() == TAMANO) {
